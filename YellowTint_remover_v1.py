...
# New functions to handle folder creation and automatic processing
...
def process_images(input_dir, output_dir):
    ...
    if not image_files:
        ...
        print(f"Please place your images in the '{os.path.basename(input_dir)}' folder and run the script again.")
        print("Supported formats: JPG, JPEG, PNG, BMP, TIFF, GIF, WEBP")
        return 0
...
if __name__ == "__main__":
    ...
    input_dir = create_directory(os.path.join(script_dir, "input_images"))
    output_dir = create_directory(os.path.join(script_dir, "output"))
    
    print("\n=== Yellow Tint Cleaner ===")
    
    # Process images if they exist
    processed = process_images(input_dir, output_dir)
    
    if processed > 0:
        print(f"\nProcessing complete! {processed} images processed successfully.")
        print(f"Output files saved to: {output_dir}")
    else:
        print("\nWaiting for images to be added to the input folder...")